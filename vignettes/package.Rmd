...
Here the package are directly refered to using `roam::fun` because the demo package _depends_ on `roam`, which does not import `roam` functions when `.onLoad` is run, so a package reference is needed for it to find the function. During `R CMD check`, the developer will also be prompted to import `roam` in `NAMESPACE`, if they choose to _depend_ on `roam`.
